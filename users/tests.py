...
class UserModelTests(TestCase):
    ...
    def test_subscription_str(self):
        ...
        self.assertEqual(str(subscription), 'user1@example.com подписан на user2@example.com')
...
class UserViewsTests(TestCase):
    ...
    def test_login_view(self):
        ...
        self.assertEqual(response.status_code, 200)  # Проверка, что остались на странице входа
        self.assertContains(response, "Неверный номер телефона или пароль.")

    def test_profile_view(self):
        self.client.login(phone_number='1234567890', password='password123')  # Вход в систему
        response = self.client.get(reverse('profile'))  # Запрос к профилю
        self.assertEqual(response.status_code, 302)  # Проверка успешного доступа к профилю

    def test_create_checkout_session_view(self):
        self.client.login(phone_number='1234567890', password='password123')  # Вход в систему
        response = self.client.post(reverse('create_subscription_session', args=[self.user2.id]))  # Создание сессии
        self.assertEqual(response.status_code, 200)
...
class SignInSerializerTest(TestCase):
    ...
    def test_validate_success(self, mock_send_code_email, mock_generate_code, mock_get_user_by_phone):
        ...
        self.assertEqual(response['detail'], "Проверочный код отправлен на указанный номер телефона.")
